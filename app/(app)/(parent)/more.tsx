  ...
  const sections: { title: string; items: MenuItem[] }[] = [
    {
      title: 'Communication',
      items: [
        { icon: 'mail-outline', label: 'Inbox', sublabel: 'Day book notes & notifications', onPress: () => router.push('/(app)/(parent)/inbox' as any) },
        { icon: 'chatbubble-ellipses-outline', label: 'Messages', sublabel: 'Chat with teachers', onPress: () => router.push('/(app)/(parent)/messages' as any) },
      ],
    },
    {
      title: 'School',
      items: [
        { icon: 'megaphone-outline', label: 'Announcements', sublabel: 'School news & updates', onPress: () => router.push('/(app)/announcements' as any) },
        { icon: 'calendar-outline', label: 'Timetable', sublabel: 'Class schedule', onPress: () => router.push('/(app)/timetable' as any) },
      ],
    },
    {
      title: 'Notifications',
        ...
        <SafeAreaView edges={['top']} style={{ backgroundColor: colors.brand.primary }}>
          <View style={styles.hero}>
            <View style={styles.heroAvatarRow}>
              <Avatar name={user?.fullName ?? 'P'} size={70} />
              <View style={styles.heroText}>
                ...
                <View style={styles.rolePill}>
                  <ThemedText style={styles.rolePillText}>PARENT</ThemedText>
                </View>
              </View>
            </View>
            {school && (
