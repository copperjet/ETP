...
import { ThemedText, Avatar, Card } from '../../../components/ui';
  ...
  const sections: { title: string; items: MenuItem[] }[] = [
    ...
    },
    {
      ...
      items: [
        ...
        ...((user?.roles ?? []).length > 1 ? [{ icon: 'swap-horizontal-outline', label: 'Switch Role', sublabel: `Active: ${user?.activeRole ?? ''}`, onPress: () => router.push('/(app)/switch-role' as any) }] : []),
        {
    ...
    <SafeAreaView style={[styles.safe, { backgroundColor: colors.background }]}>
      <ScrollView showsVerticalScrollIndicator={false}>
        ...
        <View style={[styles.profileSection, { borderBottomColor: colors.border }]}>
          <Avatar name={user?.fullName ?? 'T'} size={64} />
          <View style={styles.profileText}>
            ...
            <View style={[styles.roleBadge, { backgroundColor: colors.brand.primary + '18' }]}>
              <ThemedText variant="label" style={{ color: colors.brand.primary }}>
                {user?.activeRole?.toUpperCase() ?? 'TEACHER'}
              </ThemedText>
            </View>
          </View>
        </View>
          ...
          <View key={section.title} style={styles.section}>
            ...
            <View style={[styles.card, { backgroundColor: colors.surface, borderColor: colors.border }]}>
              {section.items.map((item, i) => (
                <MenuRow key={i} item={item} colors={colors} />
              ))}
            </View>
          </View>
        ))}
