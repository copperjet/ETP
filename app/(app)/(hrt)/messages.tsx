...
function useStreamParents(staffId: string | null, schoolId: string) {
  return useQuery<ParentOption[]>({
    queryKey: ['stream-parents', staffId, schoolId],
    enabled: !!staffId && !!schoolId,
    staleTime: 1000 * 60 * 5,
    queryFn: async () => {
      const db = supabase as any;
      // Get teacher's stream via hrt_assignments
...
export default function TeacherMessages() {
  ...
  const { data: parents } = useStreamParents(staffId, schoolId);
  ...
  return (
    <SafeAreaView style={[styles.safe, { backgroundColor: colors.background }]}>
      ...
      <FlatList
        ...
        renderItem={({ item }) => (
          <TouchableOpacity
            onPress={() => { haptics.light(); setActiveConvo(item); }}
          >
