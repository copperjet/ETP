  ...
  const sections: { title: string; items: MenuItem[] }[] = [
    {
      title: 'HR',
      items: [
        { icon: 'calendar-outline', label: 'Leave Requests', sublabel: 'View and approve leave', onPress: () => router.push('/(app)/(hr)/leave' as any) },
        { icon: 'people-outline', label: 'Staff Directory', sublabel: 'View all staff', onPress: () => router.push('/(app)/(hr)/staff' as any) },
      ],
    },
    {
      title: 'School',
      ...
      items: [
        {
          ...
          onPress: () => Alert.alert(user?.fullName ?? 'My Profile', `Role: ${user?.activeRole ?? '—'}\nSchool: ${school?.name ?? '—'}\nEmail: ${user?.email ?? '—'}`, [{ text: 'Close', style: 'cancel' }]),
        },
        ...((user?.roles ?? []).length > 1
        ...
        <SafeAreaView edges={['top']} style={{ backgroundColor: colors.brand.primary }}>
          <View style={styles.hero}>
            <View style={styles.heroAvatarRow}>
              <Avatar name={user?.fullName ?? 'H'} size={70} />
              <View style={styles.heroText}>
                ...
                <View style={styles.rolePill}>
                  <ThemedText style={styles.rolePillText}>HUMAN RESOURCES</ThemedText>
                </View>
              </View>
            </View>
            {school && (
